...
import {
  ...
  Options,
  ScenarioConfig,
} from "../lib/shell/enricher/enricher-type";

const YAML_DIR = `${__dirname}/cmd-options`;

function getOptions(scenario: ScenarioConfig) {
  const options = {
    booleans: [],
    strings: [],
    paths: [],
...
export default {
  ...
  scenarios: [\n`;
      ...
      output += `\n};`;
      writeFileSync(
        `${__dirname}/../lib/shell/enricher/enrichers/${fname.replace(
          ".yml",
          ".ts"
        )}`,
        output
  ...
  indexOutput += `]\n`;
  writeFileSync(
    `${__dirname}/../lib/shell/enricher/enrichers/index.ts`,
    indexOutput
  );
}
