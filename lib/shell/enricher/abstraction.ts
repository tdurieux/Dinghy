...
export function abstract(node: AbstractNode<any>) {
  node.traverse((node: AbstractNode<any>) => {
    if (KEEP_TYPES.includes(node.type as string)) {
      if (
        node.children.length == 1 &&
        node.children[0] instanceof AbstractValueNode
      ) {
        node.annotations.push(
          `${node.type}:${node.children[0].value.toUpperCase()}`
        );
      } else if (node instanceof AbstractValueNode) {
        node.annotations.push(`${node.type}:${node.value}`);
      }
    } else if (
      node instanceof AbstractValueNode &&
      ...
      if (
        node.children.length == 1 &&
        node.children[0] instanceof BashLiteral
      ) {
        // return false;
      }
    } else if (
