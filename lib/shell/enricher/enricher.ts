...
import { Scenario, Enricher } from "./enricher-type";
...
function parseArgWithScenario(commandArgsString: string[], scenario: Scenario) {
  ...
  if (saveLastNonOption && scenario.saveLastNonOption !== undefined) {
...
function matchArgumentAST(
  scenario: Scenario,
  strNode2AST: (x: string | void) => ShellNodeTypes[],
  argName: string
) {
...
function annotateAST(
  ...
  if (
    scenario.postProcess?.filter(
      (x) => x.subCommand !== undefined && x.subCommand == argName
    ).length ||
    0 > 0
  ) {
    return;
  ...
  let keyAST: ShellNodeTypes[] = matchArgumentAST(
    scenario,
    strNode2AST,
    argName
  );
  ...
  const valueAST: ShellNodeTypes[] = strNode2AST(value);
  ...
  if (
    ...
    for (let i = 0; i < keyAST.length; i++) {
      ...
      p.lineEnd = Math.max(valueAST[0].position.lineEnd || 0, p.lineEnd || 0);
      p.columnEnd = Math.max(
        valueAST[0].position.columnEnd || 0,
        p.columnEnd || 0
      );
    ...
    value.forEach((x) => {
      ...
      for (const node of strNode2AST(x)) {
        for (const key of keyAST) {
          ...
          p.lineEnd = Math.max(node.position.lineEnd || 0, p.lineEnd || 0);
          p.columnEnd = Math.max(
            node.position.columnEnd || 0,
            p.columnEnd || 0
          );
          key.setPosition(p);
...
function enrichAST(
  ...
  const argsASTCache = (nodeStr: string | void): ShellNodeTypes[] => {
...
export function enrich<T extends AbstractNode<any>>(root: T) {
  root.traverse(
    (node) => {
      if (node instanceof BashCommand) {
        const commandAST = node.command;
        if (!commandAST) return true;

        let command = commandAST.getElement(BashLiteral)?.value;
        ...
        const commandArgs = node.args.filter((e) => {
          return e.traverse(
            (e) => {
              if (
                e instanceof ShellAbstractValueNode &&
                e.value?.trim() == "--"
              )
                return false;
            },
